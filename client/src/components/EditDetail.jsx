...
const EditDetail = ({ address, paymethod, trackingNo, trackmethod, setPaymethod, settrackmethod, setAddress, settrackingNo, errorMsg, submitEditDetail }) => {
  ...
  return (
    <Box pad={{ horizontal: 'medium' }} animation={['fadeIn']}>
      ...
      <Box justify="between" direction="row" pad={{ vertical: 'medium' }} gap="small" align="center">
        <Text size="small" color={"red"}>{errorMsg} </Text>
        <Checkmark color={color.fontColor} onClick={() => onEditSubmit()} />
      </Box>
    </Box>
  )
}
